...
process VF_BLAST {
    ...
    """
    blastn -db ${params.vf_db} -query ${fasta} -out vf_blast.txt -evalue 1e-3 -perc_identity 85 -outfmt '6 stitle qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore' -num_threads ${task.cpus}
...
process ICE_BLAST {
    ...
    sample = fasta.getSimpleName()
    """
    blastn -db ${params.ice_db} -query ${fasta} -out ice_blast.txt -evalue 1e-3 -perc_identity 75 -outfmt '6 stitle qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore' -num_threads ${task.cpus}
