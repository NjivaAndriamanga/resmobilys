...
process DBSCAN {
    ...
    if [[ -s ${fasta} ]]; then
        set +e
        python3 ${projectDir}/bin/DBSCAN-SWA/bin/dbscan-swa.py --thread_num ${task.cpus} --diamond_thread_num ${task.cpus} --input ${fasta} --output dbscan_output
        if [[ \$? -ne 0 ]]; then
...
/*
Resistome Gene Identification with CARD
*/
process RGI {
    tag "${barID}_${type}"
    label 'rgi'
...
//Merge all plasmid type from all samples in one file
process MERGE_TYPE {
    label 'merge'
...
// Create taxa file for each plasmid
process CREATE_TAXA {
    tag "${barID}"
    ...
    output:
    path "plasmid_taxa.txt"

    script:
    """
    tail -n +2 ${plasmid_type} | cut -f1 -d\$'\t' | awk  -F'\t' '{print \$1 "\t${barID}"}' >> plasmid_taxa.txt
    """
}

//Merge all plasmid taxa from all samples in one file
process MERGE_TAXA {
    label 'merge'
...
//PLasmid clustering with mob cluster
process MOB_CLUSTER {
    label 'mob'
